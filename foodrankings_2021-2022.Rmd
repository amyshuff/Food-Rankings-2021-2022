...
charter <- district_rankings %>% 
  ...
  reshape::rename(c(Rank = "Overall Rank"))
...
There are `r nrow(charter)` charter districts that meet our ranking criteria. `r first(charter$"District Name")` is the highest performing, ranked #`r min(charter$"Overall Rank")` overall.
...
A downloadable spreadsheet of statewide 2022 ratings and demographic information is available.

https://tea.texas.gov/sites/default/files/statewide_overall_accountability_ratings_2022.xlsx
...
car.score <- district %>% 
  filter(tea.all.stud>=10000 & eco.dis.pct >= 60) %>% 
  mutate(district_sum = ((Pct.Lunch.Participation*0.25)+(Pct.Breakfast.Participation*0.5)+(cacfp_supper01*0.1)+(snack_anyafter*0.15))) %>% 
  select(District.Number, district_sum) %>%
  rename(car.score = district_sum)

scores <- left_join(car.score, tea.score, by = "District.Number")

ggplot(data = scores, aes(x=car.score, y = tea.progress)) +
  geom_point() +
  geom_smooth(method = lm)

ggplot(data = scores, aes(x=car.score, y = tea.relative.performance)) +
  geom_point() +
  geom_smooth(method = lm)


#BEST CORRELATION is with the tea growth score
...
ggplot(data = scores, aes(x=car.score, y = tea.gap)) +
  geom_point() +
  geom_smooth(method = lm)


ggplot(data = scores, aes(x=car.score, y = tea.achievement.score)) +
  geom_point() +
  geom_smooth(method = lm)

ggplot(data = scores, aes(x=car.score, y = tea.overall.score)) +
  geom_point() +
  geom_smooth(method = lm)
...
car.school.rankings <- read_xlsx("Districts.xlsx") %>% 
  rename(car.school.rankings = "Average of Total Score",
         District = Districts)

scores <- left_join(scores, car.school.rankings)

ggplot(data = scores, aes(x=car.score, y = car.school.rankings)) +
  geom_point() +
  geom_smooth()
